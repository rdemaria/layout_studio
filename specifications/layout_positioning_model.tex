...
or all four are absent. The transformations in \texttt{magnetic\_center} and
\texttt{frames} have no explicit reference: their implicit reference is the
type center. An absent magnetic group contributes no magnetic frames. An
absent shape or feature group is represented by omitted JSON members;
explicit \texttt{null} values are invalid. Beam-interface fields are not
permitted on types.
...
transformation has no explicit reference: it starts at the object's center.
Its local \texttt{ts} operations use the type's mechanical path, just as for
other local center transformations.
...
has only its implicit \texttt{center} frame.
...
For feature $j\in\{m,b\}$, denoting the type magnetic axis and an object's
effective beam interface respectively, define its path operator by
\begin{equation}
  \Path_j(\Frame,d)
  =\operatorname{Advance}(\Frame,d,\kappa_j d,\omega_j),
  \label{eq:feature-path}
\end{equation}
where $\ell_j>0$, $\kappa_j$, and $\omega_j$ are the feature's effective
length, curvature, and roll. Magnetic values belong to $T$; beam values belong
to $O$ or are inherited from $T$.  These values are independent of the optional shape's
mechanical path.  The finite feature axis is the locus of frame origins
$\operatorname{origin}(\Path_j(\Frame_j,u))$ for
...
For object $O$ of type $T$, let $\ops_b$ be its effective beam-center
transformation. Its beam frames, in object-local coordinates, are
\begin{align}
  \Frame_{O.\mathrm{beam\_center}}^{L}
    &=\Apply_T(\identity,\ops_b),\\
  \Frame_{O.\mathrm{beam\_entry}}^{L}
    &=\Path_b\!\left(
       \Frame_{O.\mathrm{beam\_center}}^{L},-\ell_b/2\right),\\
  \Frame_{O.\mathrm{beam\_exit}}^{L}
    &=\Path_b\!\left(
       \Frame_{O.\mathrm{beam\_center}}^{L},+\ell_b/2\right).
  \label{eq:beam-frames}
\end{align}

When the object beam group is omitted and the magnetic group exists,
$\ops_b=\ops_m$ and $(\ell_b,\kappa_b,\omega_b)=(\ell_m,\kappa_m,\omega_m)$.
Consequently, for $q\in\{\mathrm{center},\mathrm{entry},\mathrm{exit}\}$,
\begin{equation}
  \Frame_{O.\mathrm{beam\_}q}^{L}
  =\Frame_{T.\mathrm{magnetic\_}q}.
...
include offsets and rotations. An absent magnetic group removes its three
magnetic frames. Beam frames and their finite axis exist exactly when the
object has an explicit beam group or can inherit a magnetic group.
...
$a$ (for example \texttt{center}, \texttt{magnetic\_entry}, \texttt{beam\_entry}, or a stored named
...
frame $\Frame_{O.a}^{L}$ using Section~\ref{sec:type-local}. For stored and
magnetic frames this equals $\Frame_{T.a}$; beam frames use the object's
effective interface. A conditional implicit frame is a valid target only
...
\end{equation}
For a stored frame, $\Frame_{O.f}^{L}=\Frame_{T.f}$ is given by
