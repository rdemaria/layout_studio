...
\author{}
\date{Prototype specification --- 4 September 2026}
...
obtain world frames, swept object geometry, Beam entry and exit planes, and a
curve station inferred from an arbitrary world point.
\end{abstract}
...
\begin{itemize}
  ...
  \item A \emph{type} owns reusable physical geometry, its local curved axis,
        Beam-axis metadata, and named local frames.
...
ts & advance along a curve or type-local curved path & m \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{lstlisting}
{
  ...
  "magnetic_length": <positive number>,
  "frames": {
    "name": {"transformation": [...]}
  }
}
\end{lstlisting}
Its shape is exactly one of
\begin{align*}
...
The local transformations in \texttt{magnetic\_center} and \texttt{frames}
have no explicit reference: their implicit reference is the type center.
Stored frame names may not use the reserved implicit frame names
\begin{equation}
  \texttt{center},\quad
...
\end{lstlisting}
Shape, color, Beam-axis data, and named local frames belong to the type, not to
the instance.  Any number of objects may instantiate the same type.
...
\section{Type-local curved geometry and frames}
\label{sec:type-local}

\subsection{The type path operator}

For a type $T$ with shape curvature $\kappa_T$ and roll $\omega_T$, define
\begin{equation}
  \Path_T(\Frame,d)
...
The Beam-axis definition consists of a magnetic-center list $\ops_m$ and a
positive magnetic length $\ell_m$.  The derived local frames are
\begin{align}
  \Frame_{T.\mathrm{magnetic\_center}}
    &=\Apply_T(\identity,\ops_m),\\
  \Frame_{T.\mathrm{magnetic\_entry}}
    &=\Apply_T\!\left(\identity,
       \ops_m\mathbin{+}[\texttt{ts},-\ell_m/2]\right),\\
  \Frame_{T.\mathrm{magnetic\_exit}}
    &=\Apply_T\!\left(\identity,
       \ops_m\mathbin{+}[\texttt{ts},+\ell_m/2]\right).
  \label{eq:magnetic-frames}
\end{align}
Here $+$ denotes list concatenation, not matrix addition.  The entry and exit
planes pass through the corresponding origins and are spanned by the local
$\mathbf{x}$ and $\mathbf{y}$ axes; their normal is the local tangent
$\mathbf{t}$.  The magnetic length is not required to equal or fit within the
physical shape length, and the magnetic-center transform may include offsets
or rotations.
...
target frame $\Frame_D^W$.  Then obtain the target's type-local operation list
$\ops_a$ using Section~\ref{sec:type-local}.

The object center world frame is the unique frame satisfying
\begin{equation}
  \Apply_T(\Frame_O^W,\ops_a)=\Frame_D^W.
  \label{eq:target-constraint}
\end{equation}
It is computed as
\begin{equation}
  \boxed{\Frame_O^W=\Apply_T(\Frame_D^W,\ops_a^{-1})}
...
\begin{equation}
  \boxed{\Frame_{O.f}^W=\Apply_T(\Frame_O^W,\ops_f)}
  \label{eq:frame-world-frame}
\end{equation}
and all shape points follow from Equations~\eqref{eq:box-sweep} or
...
\begin{enumerate}
  ...
  \item Resolve its named and Beam frames using
        Equation~\eqref{eq:frame-world-frame}, and sweep its shape in world
        coordinates using Section~\ref{sec:type-local}.
\end{enumerate}
...
\begin{itemize}
  ...
  \item an entity name is empty, an operation value is not finite, a curve
        segment length is not positive, a shape dimension is not positive, or
        the magnetic length is not positive;
  \item a shape tuple is not exactly one of the two canonical forms
        \begin{align*}
          &[\texttt{"box"},d_x,d_y,d_z,\kappa,\omega],\\
          &[\texttt{"cylinder"},r,d_z,\kappa,\omega];
        \end{align*}
  \item a color is not a six-digit hexadecimal color;
  \item a type, curve, object, named frame, or positioning target does not
        exist;
  \item a stored frame uses an implicit reserved name;
...
reference curve.  The reusable type also defines two local frames by following
its type path from the center.
...
\begin{align*}
 ...
 \Frame_{Q1.\mathrm{end}}^W
   &=\Path_{\mathrm{magnet}}(\Frame_{Q1.\mathrm{center}}^W,+1).
\end{align*}
Since both the curve and type curvature are zero, the three origins are
...
requires both.  Beam entry and exit are likewise frames;
their $x$--$y$ planes are the physically meaningful transverse planes.  A later
beam-physics export may use these frames to construct element boundaries and
derive survey offsets or misalignments without changing the installation
layout itself.
